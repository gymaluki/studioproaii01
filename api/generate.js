...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (data.error) {
      ...
      let msg = m;
      if (m.includes('API_KEY_INVALID')||m.includes('invalid')) msg='API Key inválida. Verifique se copiou corretamente.';
      else if (m.includes('quota')||m.includes('QUOTA')) msg='Limite de uso atingido. Tente amanhã.';
      else if (m.includes('SAFETY')) msg='Conteúdo bloqueado pelos filtros de segurança.';
